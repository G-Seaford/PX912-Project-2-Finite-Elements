...
class QuadMesh(BaseMesh):
    ...
    def __init__(self, domain, Nx, Ny):
        """
        domain: Domain object with domain.vertices (4 points in CCW order).
        Nx, Ny: Number of elements in the horizontal (x) and vertical (y) directions
    ...
    def generate_mesh(self, max_volume=None):
        ...
        from domain corners.  The max_volume is unused here, but we keep 
        the signature for consistency.
        """
    ...
    def element_stiffness(self, elem_idx, plane_thickness, E, nu, nodes, mesh_elements):
        ...
        Returns an 8×8 K_e array (4 nodes × 2 dof each).
        """
        ...
        for i, xi in enumerate(points):
            for j, eta in enumerate(points):
                ...
                B[0, 0::2] = dN_global[:, 0]  # ∂N/∂x for each node
                B[1, 1::2] = dN_global[:, 1]  # ∂N/∂y for each node
                B[2, 0::2] = dN_global[:, 1]  # ∂N/∂y in the x columns
                B[2, 1::2] = dN_global[:, 0]  # ∂N/∂x in the y columns 
...
class FiniteElement:
    ...
    def _set_system_params(self, domain, E, nu, thickness, tractions=None,init_vol=None, Nx=None, Ny=None):
    ...
    def compute_element_stress_strain(self, elem_idx):
        ...
        if self.mesh_type == "triangular":
            ...
            N, dN = self.mesh_obj.shape_funcs(xi, eta)
            ...
            num_nodes_elem = len(nodes_idx)
            B = np.zeros((3, 2 * num_nodes_elem))
            
            for i in range(num_nodes_elem):
                B[0, 2 * i] = dN_global[i, 0]
                B[1, 2 * i + 1] = dN_global[i, 1]
                B[2, 2 * i] = dN_global[i, 1]
                B[2, 2 * i + 1] = dN_global[i, 0]
    ...
    def _adaptive_refinement(self, tol=0.01, max_iter=10):
        ...
        if self.mesh_type == "triangular":
            ...
            # Build product Gauss-Legendre
            points_1D, weights_1D = leggauss(3)
            quad_points = []
            quad_weights = []
            ...
            quad = GaussLegendreQuad(np.array(quad_points), np.array(quad_weights))
